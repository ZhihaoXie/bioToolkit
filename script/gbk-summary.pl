...
my ($all_gene,$all_length,$all_avg,$all_rrna,$all_trna,$all_gc,$gc);
$all_rrna=0;
$all_trna=0;
my $sall_length;
my $all_density = 0;
...
while (my $seq=$seq_obj->next_seq()) {
    my $num_cds=0;my $length_cds=0;my $num_rrna=0;my $num_trna=0;my $avg_cds=0;my $gc_num=0;my $gc_cds=0;
    my $density = 0;
    my $id = $seq->id;
    ...
    for my $f (@features) {
        if ($f->primary_tag eq 'CDS' || $f->primary_tag eq 'rRNA' || $f->primary_tag eq 'tRNA') {
            $num_cds += 1;
            $all_gene += 1;
            my $l_cds = $f->length;
            $length_cds += $l_cds;
    ...
    printf "%0.2f\t%0.2f%%\t%0.2f%%\t%0.2f", $avg_cds,$ratio,$gc_cds,$density;
    print "\t$num_rrna\t$num_trna\n";
}
if ($all_gene > 0) {
    $all_avg = $all_length/$all_gene;
...
printf "%0.2f\t%0.2f%%\t%0.2f%%\t%0.2f", $all_avg,$all_ratio,$gc,$all_density;
